...
$shopcredits = dbSelect('*', 'cms_shop', "WHERE art = 'credits' ORDER BY count ASC");
while ($row = $shopcredits->fetch_array()) {
	$tpl->block_assign('shopcredits', array(
...
if(isset($_POST['submit'])){
	...
	if(empty($shopid)){
		...
		$error = 2;
	} else if(CheckShop($user->UserData('id'), "credits")){
		$error = 3;
	} else {
...
$tpl->display($user->UserData('theme').'/page-credits');
