...
<html>
<head>
	<title>Habbo Client - {USERNAME}</title>
...
{$("#audio_"+ id).remove();$("#client-ui").append('<audio controls autoplay id="audio_'+ id+'" style="width: 1px;height: 1px;z-index: 1px;"></audio>');$("#client-ui #audio_"+ id).append("<source src='"+ url+"' type='audio/mp3'>");console.log("VOLUME: "+ volume);document.getElementById("audio_"+ id).volume=(volume/100);}
...
window.onload = bz_punkt();
	...
	<script type="text/javascript">
		var flashvars = {
			...
			"conection.info.host" : "5.196.135.254", 
			...
			"connection.info.host" : "5.196.135.254",
			"has.identity" : "1", 
    ...
    $("#client-ui").append("<div id='Fenster-Agario' onmousedown='dragStart(this);' class='alert' onClick='makezindex(\"Agario\")' style='left: " + left + "px;top: " + top + "px;width: 500px;height: 450px;'></div>");
    ...
    break;
					case "55":
						...
						switch(text[1])
						{
        					...
        					if(soundid !== 0)
        					{
        						playSound('/swf/hof_furni/mp3/sound_machine_sample_' + soundid + '.mp3', text[1]);
        					}
        				break;
...
<script type="text/javascript">
		...
		window.onload = function(){
			...
			{
				$('#USERONLINE').html('');
				$('#USERONLINE').load('{url}/lib/standard/useronlinestunden.tpl.php').fadeIn("slow");
			}, 60000);
		}
	</script>-->
...
<div style="position: absolute;top: 0px;right: 46%;padding: 8px 18px;border-radius: 0px 0px 3px 3px;border-top: none;background-color: rgb(50, 49, 46);border: 2px solid rgb(84, 82, 77);color: rgb(255, 255, 255);border-top: none;font-family: ubuntu;text-shadow: 0px 0px 1px rgb(113, 113, 113);font-size: 14px;z-index: 99999;">
		<center><b id="useronline">{ONLINE}</b> Habbos online</center>
		</div>
  ...
  <div id="bz_notification_desc">User hat dich geküsst</div>
...
<script type="text/javascript">
  ...
  switch ($rnd_pic) {
		case '1':
		echo 'http://i.imgur.com/aflvHTw.png';
            break;
