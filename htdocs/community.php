...
$userbans = dbSelect('*', 'bans', "WHERE bantype='user' AND added_by != 'Autoban System' ORDER BY id DESC LIMIT 4");
...
$premiums = dbSelect('*', 'users', "WHERE rank > 0 ORDER BY id DESC LIMIT 12");
while ($row = $premiums->fetch_array()) {
	$tpl->block_assign('premiums', array(
		'ID'		=>	$row['id'],
		'USERNAME'	=>	$row['username'],
		'USERLOOK'	=>	$row['look'],
		'USERONLINE'=>	$row['online'],
...
$tpl->assign(array(
	...
	'NEWSLIDE'	=> 	$newsslide,
	'SEITE'		=>	"Community",
	'HOTROOMSR'	=>	$hotrooms->num_rows,
	'USERSBIRTHR'	=>	$usersbirths->num_rows
));

$tpl->display($user->UserData('theme').'/page-community');
